...
import com.edward.nyansapo.databinding.ActivityWordAssessmentBinding
...
@AndroidEntryPoint
class WordAssessmentFragment : Fragment(R.layout.activity_word_assessment) {
    private val TAG = "ParagraphAssessmentFrag"

    private lateinit var binding: ActivityWordAssessmentBinding
    private val navArgs: WordAssessmentFragmentArgs by navArgs()
    private val viewModel: WordAssessmentViewModel by viewModels()

    private val RC_PERMISSION = 4
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        binding = ActivityWordAssessmentBinding.bind(view)
        initProgressBar()
        setDefaults()
    ...
    private fun subscribeToObservers() {
        viewLifecycleOwner.lifecycleScope.launchWhenResumed {
            launch {
                viewModel.wordAssessmentEvents.collect {
                    when (it) {
                        is Event.PermissionGranted -> {
                            viewModel.setEvent(Event.FetchWords)
                            setOnClickListener()
                            animateArrow()
            ...
            launch {
                viewModel.fetchWordsStatus.collect {
                    Log.d(TAG, "subscribeToObservers: fetchParagraphStatus:${it.status.name}")
                    when (it.status) {
                        Resource.Status.SUCCESS -> {
                            viewModel.setEvent(Event.FetchSingleWord)
    ...
    private fun goToThankYou(assessment: Assessment) {
        findNavController().navigate(WordAssessmentFragmentDirections.actionWordAssessmentFragmentToThankYouFragment(assessment, navArgs.student))
    }

    private fun goToLetter(assessment: Assessment) {
        findNavController().navigate(WordAssessmentFragmentDirections.actionWordAssessmentFragmentToLetterAssessmentFragment(assessment, navArgs.student))

    }


    private fun deactivateTranscriptionColor() {
        binding.assessmentCard!!.background = originalReadButtonDrawable
        binding.assessmentCard!!.setTextColor(Color.BLACK)
    ...
    private fun setWord(data: String) {
        binding.assessmentCard!!.setText(data)
    }
    ...
    private fun activateTranscriptionColor() {
        originalReadButtonDrawable = binding.assessmentCard!!.background
        ...
        binding.assessmentCard!!.background = newDrawable
        binding.assessmentCard!!.setTextColor(lightbrown)
    ...
    sealed class Event {
        ...
        data class GoToLetter(val assessment: Assessment) : Event()
        ...
        object FetchWords : Event()
        object FetchSingleWord : Event()
